...
export class StyledStackCardEditor extends LitElement {
  ...
  protected firstUpdated() {
    StyledStackCard.ensureHaEditorElements();
  }
  ...
  private _styleSchema(preset: string) {
    ...
    if (preset === 'custom') {
      schema.push(
        ...
        { name: 'angle', selector: { number: { min: 0, max: 360, step: 1, unit_of_measurement: '°' } } }
      );
    }
  ...
  private _handleMove(ev: Event) {
    if (!this._config.cards) return;
    const move = (ev.currentTarget as HTMLElement & { move?: number }).move ?? 0;
  ...
  render() {
    ...
    return html`
      <div class="card-config">
        ...
        <div class="toolbar">
          <ha-tab-group @tab-changed=${this._handleSelectedCard}>
            ${cards.map(
              (_card, i) => html`
